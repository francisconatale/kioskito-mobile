...
export const Analytics = ({ products, sales }) => {
    ...
    return (
        <ScrollView style={{ flex: 1, padding: 16 }}>
            <View style={{ backgroundColor: 'white', padding: 16, borderRadius: 12, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2, marginBottom: 16 }}>
                ...
                <View style={{ backgroundColor: '#eff6ff', padding: 16, borderRadius: 8, marginBottom: 12 }}>
                    <Text style={{ fontSize: 14, color: '#6b7280', marginBottom: 4 }}>Ingresos totales</Text>
                    <Text style={{ fontSize: 30, fontWeight: 'bold', color: '#3b82f6' }}>${totalSalesMonth.toLocaleString()}</Text>
                </View>

                <View style={{ flexDirection: 'row', gap: 12 }}>
                    <View style={{ flex: 1, backgroundColor: '#d1fae5', padding: 12, borderRadius: 8 }}>
                        <Text style={{ fontSize: 12, color: '#6b7280' }}>Ventas hoy</Text>
                        <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#10b981' }}>${totalSalesToday}</Text>
                    </View>
                    <View style={{ flex: 1, backgroundColor: '#f3e8ff', padding: 12, borderRadius: 8 }}>
                        <Text style={{ fontSize: 12, color: '#6b7280' }}>Promedio por venta</Text>
                        <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#9333ea' }}>
                            ${sales.length > 0 ? Math.round(totalSalesMonth / sales.length) : 0}
                        </Text>
                    </View>
                </View>
            </View>
